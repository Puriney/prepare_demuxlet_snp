...
# option_list = list(
#   make_option(c("--inputfiles"), action="store", default=NULL, type='character',
#               help="file1.txt:file2.txt:file3.txt"),
#   # make_option(c("--techRep"), action="store", default=NULL, type='character',
#   #             help="/path/to/manifest/file/containing/techRepInfo.txt"),
#   make_option(c("--snp"), action="store", default=NULL, type='character',
#               help="/path/to/All_SNPs_Merged_mergedPositions.bed"),
#   make_option(c("--outfile"), action="store", default=NULL, type='character',
#               help="/path/to/exportfile.txt")
# )
# opt = parse_args(OptionParser(option_list=option_list))
# 
# if(is.null(opt$inputfiles) || is.null(opt$snp) || is.null(opt$outfile)){
#   stop()
# }
...
print(opt)
if (length(opt) <= 3) {stop()}
snp_file <- opt[1]
outfile <- opt[2]

fileList <- opt[3:length(opt)]
# snp_file <- opt$snp # "../vcf/All_SNPs_Merged_mergedPositions.bed" # Yun made this as the script's input
# out_dir <- dirname(opt$outfile)

# snp_file <- '/volumes/lab/users/yyan/project/snubar_atac/misc_analysis/demuxplet_res/allrep1/vcf/All_SNPs_Merged_mergedPositions.bed'
# outfile <- '/volumes/lab/users/yyan/project/snubar_atac/misc_analysis/demuxplet_res/allrep1/demuxlet/demuxlet_input.txt'
# fileList <- c("/volumes/lab/users/yyan/project/snubar_atac/misc_analysis/demuxplet_res/allrep1/birdseed/k562.affx","/volumes/lab/users/yyan/project/snubar_atac/misc_analysis/demuxplet_res/allrep1/birdseed/mdamb231.affx","/volumes/lab/users/yyan/project/snubar_atac/misc_analysis/demuxplet_res/allrep1/birdseed/mdamb436.affx")
...
# fileList <- unlist(strsplit(x = as.character(opt$inputfiles), split = ':'))
print(fileList)
...
colnames(depth) <- gsub(pattern = ".affx", replacement = "", x = basename(fileList))
...
#--------------------------------------------------------------------
print("=== preallocate a matrix to hold the number of differences between a given sample and all other samples")
print(ncol(nonZero_call))
...
# snp_file <- "../vcf/All_SNPs_Merged_mergedPositions.bed" # Yun made this as the script's input
...
#reorder snps to be in chromosome numeric order -- sort VCF 
...
colnames(demuxlet_df)[1] <- paste0("#",colnames(demuxlet_df)[1]) # VCF sample name row
# write.table(x = demuxlet_df, file = opt$outfile, row.names = FALSE, col.names = TRUE, sep = "\t", quote = FALSE)
write.table(x = demuxlet_df, file = outfile, row.names = FALSE, col.names = TRUE, sep = "\t", quote = FALSE)
# Attach the VCF header
