...
option_list = list(
  ...
  make_option(c("--outsnp"), action="store", default=NULL, type='character',
              help="/path/to/inputMpileupMatrix.snp"),
  make_option(c("--outaffx"), action="store", default=NULL, type='character',
              help="/path/to/inputMpileupMatrix.affx")
)

opt = parse_args(OptionParser(option_list=option_list))

if(is.null(opt$input) || is.null(opt$affx) || is.null(opt$outsnp) || is.null(opt$outaffx) ){
  # print usage information if an argument is not provided
  stop('Arguments are missings')
}

stopifnot(dirname(opt$outsnp) == dirname(opt$outaffx))

outdir <- dirname(opt$outsnp)
setwd(outdir)
AFFX_file <- opt$affx
mpileup_file <- opt$input
snp_outfile <- opt$outsnp # gsub(".vcf",".snp",paste(opt$outdir,"/",basename(mpileup_file),sep=""))
affx_outfile <- opt$outaffx #gsub(".vcf",".affx",paste(opt$outdir,"/",basename(mpileup_file),sep=""))
...
#if snp_outfile exists, skip this step
if(!file.exists(snp_outfile)) {
  for (j in 1:nrow(vcf))
  {
    if(j %% 10000 == 0)
    {
